...
const AfterLogin = () => {
  ...
  const handleViewProduct = (productId) => {
    ...
    if (selectedProduct) {
      ...
      navigate({
        pathname: `/viewproduct/${productId}`,
        state: { product: rest },
      });
    }
  };

  const handleBuyProduct = (event, productId) => {
  ...
  const handleAddToWishList = async (event, productId) => {
    ...
    try {
      ...
      console.log(selectedProduct.productAdd);

      const response = await axios.post(
        "http://localhost:8080/productAddToWishList",
        selectedProduct,
        {
          headers: {
            Authorization: `Bearer ${authToken}`,
      ...
      // Navigate to wishlist page after updating the state
  ...
  const handleAddToCompare = async (event, productId) => {
    event.stopPropagation();
    try {
      const authToken = localStorage.getItem("token");
      console.log(authToken);
      ...
      const response = await axios.post(
        "http://localhost:8080/productAddToCompare",
        selectedProduct,
        {
          headers: {
            Authorization: `Bearer ${authToken}`,
  ...
  return (
    <>
      ...
      >
        ...
        <div
          style={{ marginLeft: "18%", marginRight: "5%", marginTop: "120px" }}
        >
